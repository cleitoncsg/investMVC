...
int main() {
    ...
    pSuite = CU_add_suite("minimosQuadradosTeste", init_suite, clean_suite);
    if (NULL == pSuite) {
        CU_cleanup_registry();
        return CU_get_error();
    }
    ...
    if ((NULL == CU_add_test(pSuite, "testCalculoCoeficienteAngular", testCalculoCoeficienteAngular)) ||
            (NULL == CU_add_test(pSuite, "testCalculoCoeficienteLinear", testCalculoCoeficienteLinear))) {
        CU_cleanup_registry();
        return CU_get_error();
    }
