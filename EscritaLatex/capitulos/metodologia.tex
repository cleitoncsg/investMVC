...
Segundo \citeonline{yin}, a validade da construção está presente na fase de coleta de dados do Estudo de Caso. Posteriormente, os dados podem ser analisados para responder a questão de pesquisa do trabalho.
...
Este trabalho não possui validade externa, visto que as negociações são exclusivas para o Mercado de Moedas no par de negociações euro-dólar com as demais configurações descritas na Seção \ref{sec:projetoEstudo}.

\subsubsection{Validade Interna}
Para \citeonline{yin}, a validade interna de um Estudo de Caso é gerada pela técnica de triangulação em que é usada várias fontes de dados e métodos de coleta para confirmar se os resultados convergem. 
...
Os agentes de software possuem um método indispensável chamado \textit{action}, é um método sem retorno que ocorre logo que o agente de software é instanciado. Não foi possível realizar testes unitários para este tipo de método usando a ferramenta JUnit mesmo com o uso de \textit{Mock Objects}.
